 ...
 *
 * `__http(requestJSON, onChunk)` creates its Promise via xsBridgePromise (which
 ...
 *   - streams each response chunk back via xsBridgeEmitToken -> onChunk(text)
 *   - settles with xsBridgeComplete({status, headers}) on completion / error.
 *
...
static void xs_http(xsMachine* the)
{
    ...
    uint32_t id = xsBridgePromise(the, &xsArg(1));  /* roots onChunk */
